...
public class PlayCustomVideoCommand {
    ...
    public static int execute(CommandContext<CommandSource> context, boolean hasControlBlocked, boolean hasCanSkip, boolean hasPartialMode, boolean hasOptions) {
        // Retrieve the arguments
        Collection<ServerPlayerEntity> target;
        try {
            target = EntityArgument.getPlayers(context, "target");
        } catch (CommandSyntaxException e) {
            context.getSource().sendFailure(new StringTextComponent("Invalid target"));
            return 0;
        }
